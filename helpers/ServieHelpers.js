...
module.exports = {
    ...
    addServie: (serviceData) => {
        return new Promise((resolve, reject) => {
            db.get().collection(collection.SERVICE_COLLECTION).insertOne(serviceData).then((status) => {
    ...
    serviceExist: (serviceName) => {
        return new Promise(async (resolve, reject) => {
            await db.get().collection(collection.SERVICE_COLLECTION).findOne({ service: serviceName }).then((service) => {
    ...
    getService: () => {
        return new Promise(async (resolve, reject) => {
            await db.get().collection(collection.SERVICE_COLLECTION).find().toArray().then((serviceData) => {
    ...
    updateService: (serviceData) => {
        ...
        return new Promise(async (resolve, reject) => {
            await db.get().collection(collection.SERVICE_COLLECTION).updateOne({ _id: objId(serviceData.id) },
                {
                    $set: {
    ...
    categoryExist: (cat, collection) => {
        console.log(cat)
        return new Promise(async (resolve, reject) => {
            await db.get().collection(collection).findOne({ category: cat }).then((status) => {
                status ? resolve(true) : resolve(false)
            })
        })
    },
    updateStatus: (cat, serviceId, collection) => {
        ...
        return new Promise(async (resolve, reject) => {
            await db.get().collection(collection).updateOne({ category: cat },
                {
                    $push: {
                        relation: serviceId
                    }
    ...
    getAprovedService: () => {
        return new Promise(async (resolve, reject) => {
            await db.get().collection(collection.SERVICE_COLLECTION).find({ status: "approved" }).toArray().then((status) => {
